...
public class ImageResultAdapter extends BaseAdapter {
    ...
    private HashMap<CustomImage, ImageResultBlock> map;
     ...
     */
    public ImageResultAdapter(Context context, Collection<CustomImage> entries) {
        super();
        mContext = context;
        data = new ArrayList<>();
        map = new HashMap<>();
    ...
    public ConstraintLayout getView(int position, View convertView, ViewGroup parent) {
        ...
        if (map.get(link) != null) {
            ...
            ImageResultBlock block = new ImageResultBlock(parent, mContext, link, map);
            map.put(link, block);
            cons = block.getView();
        }
        return cons;
